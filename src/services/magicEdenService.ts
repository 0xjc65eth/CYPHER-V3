...
export interface MagicEdenCollectionDetail {
  symbol: string;
  name: string;
  description?: string;
  imageURI?: string;
...
export interface MagicEdenTokensResponse {
  tokens: MagicEdenToken[];
  total?: number;
}

export interface MagicEdenBlockActivity {
  kind: string;
  tokenId: string;
  inscriptionNumber?: number;
  collectionSymbol?: string;
...
export interface MagicEdenBlockActivitiesResponse {
  activities: MagicEdenBlockActivity[];
  total?: number;
}

export interface MagicEdenRareSatListing {
  id: string;
  utxo?: string;
  price?: number;
  seller?: string;
...
export interface MagicEdenRareSatListingsResponse {
  listings: MagicEdenRareSatListing[];
  total?: number;
}

export interface MagicEdenRareSatUtxo {
  txid: string;
  vout: number;
  value: number;
  satRanges: Array<{
...
export interface MagicEdenRareSatUtxosResponse {
  utxos: MagicEdenRareSatUtxo[];
  total?: number;
}

export interface MagicEdenBatchListingPSBTRequest {
  listings: Array<{
    utxo: string;
    price: number;
...
export interface MagicEdenBatchListingPSBTResponse {
  psbt: string;
  signIndexes: number[];
  expiration?: string;
}

export interface MagicEdenBatchListingSubmitRequest {
  psbt: string;
  signatures: string[];
}

export interface MagicEdenBatchListingSubmitResponse {
  txId?: string;
  listingIds?: string[];
  success: boolean;
  message?: string;
...
export interface MagicEdenTokensParams {
  collectionSymbol?: string;
  ownerAddress?: string;
...
export interface MagicEdenBlockActivitiesParams {
  kind?: string;
  collectionSymbol?: string;
  limit?: number;
  offset?: number;
...
export interface MagicEdenRareSatListingsParams {
  limit?: number;
  offset?: number;
  sortBy?: string;
  sortDirection?: 'asc' | 'desc';
...
export interface MagicEdenServiceError {
  status: number;
  message: string;
  endpoint: string;
  timestamp: number;
...
export class MagicEdenService {
   ...
   */
  async getRareSatListings(params: MagicEdenRareSatListingsParams = {}): Promise<MagicEdenRareSatListingsResponse> {
    const queryParts: string[] = [];
    ...
    const cached = this.getCached<MagicEdenRareSatListingsResponse>(cacheKey);
    if (cached) return cached;

    try {
      const data = await this.request<MagicEdenRareSatListingsResponse>(
      ...
      console.error('[MagicEdenService] Failed to get rare sat listings:', error);
      return { listings: [] };
    }
  }
   ...
   */
  async getRareSatsByWallet(address: string): Promise<MagicEdenRareSatUtxosResponse> {
    const cacheKey = `raresats-wallet-${address}`;
    const cached = this.getCached<MagicEdenRareSatUtxosResponse>(cacheKey);
    if (cached) return cached;

    try {
      const data = await this.request<MagicEdenRareSatUtxosResponse>(
      ...
      console.error(`[MagicEdenService] Failed to get rare sats for wallet "${address}":`, error);
      return { utxos: [] };
    }
  }
  ...
  async getBatchListingPSBT(
    body: MagicEdenBatchListingPSBTRequest
  ): Promise<MagicEdenBatchListingPSBTResponse | null> {
    try {
      const data = await this.request<MagicEdenBatchListingPSBTResponse>(
      ...
      console.error('[MagicEdenService] Failed to get batch listing PSBT:', error);
      return null;
    }
  }
  ...
  async submitBatchListing(
    body: MagicEdenBatchListingSubmitRequest
  ): Promise<MagicEdenBatchListingSubmitResponse> {
    try {
      const data = await this.request<MagicEdenBatchListingSubmitResponse>(
      ...
      console.error('[MagicEdenService] Failed to submit batch listing:', error);
      return {
        success: false,
