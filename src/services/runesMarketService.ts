 ...
 *
 * Migration status (Hiro primary with fallback):
...
const CONFIG = {
  ...
  FALLBACK_URL: 'https://api.hiro.so',
...
export interface RunesApiError {
  error: string;
  message?: string;
  statusCode?: number;
}
...
export class RunesMarketService {
  ...
  private getMEHeaders(): Record<string, string> {
    const meApiKey = process.env.GAMMA_API_KEY || '';
    const headers: Record<string, string> = {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
    };
   ...
   * These must use Gamma.io directly since Hiro doesn't provide them.
   */
  ...
  private async executeWithRetry<T>(
    ...
    const baseUrl = isMarketplace ? CONFIG.FALLBACK_URL : CONFIG.BASE_URL;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorBody: RunesApiError | null = null;
        try {
   ...
   * Hiro primary, Gamma.io fallback
   */
  async getRuneMarketInfo(rune: string): Promise<RuneMarketInfo> {
    ...
    try {
      ...
      // Hiro failed, fall back to Gamma.io
    }
...
export const runesMarketService = new RunesMarketService();
