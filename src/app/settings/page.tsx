                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => setTheme('dark')} className={`px-3 py-1.5 text-xs font-mono rounded-lg transition-colors ${theme === 'dark' ? 'bg-[#F7931A]/20 text-[#F7931A] border border-[#F7931A]/30' : 'bg-white/5 text-white/40 border border-white/10'}`}>Dark</button>
                      <button onClick={() => setTheme('light')} className={`px-3 py-1.5 text-xs font-mono rounded-lg transition-colors ${theme === 'light' ? 'bg-[#F7931A]/20 text-[#F7931A] border border-[#F7931A]/30' : 'bg-white/5 text-white/40 border border-white/10'}`}>Light</button>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {(['compact', 'comfortable', 'spacious'] as const).map((d) => (
                        <button key={d} onClick={() => setDensity(d)} className={`px-3 py-1.5 text-xs font-mono rounded-lg capitalize transition-colors ${density === d ? 'bg-[#F7931A]/20 text-[#F7931A] border border-[#F7931A]/30' : 'bg-white/5 text-white/40 border border-white/10'}`}>{d}</button>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-[#0a0a14] border border-[#1a1a2e] rounded-xl p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs font-mono text-white outline-none focus:border-[#F7931A]/50">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <select value={language} onChange={(e) => setLanguage(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs font-mono text-white outline-none focus:border-[#F7931A]/50">
              ...
              <div className="bg-[#0a0a14] border border-[#1a1a2e] rounded-xl p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-white font-mono">2FA Status: <span className={twoFaEnabled ? 'text-green-400' : 'text-red-400'}>{twoFaEnabled ? 'Enabled' : 'Disabled'}</span></p>
                      <p className="text-xs text-white/40">Authenticator app (TOTP)</p>
                    </div>
                  </div>
                  <button onClick={() => setTwoFaEnabled(!twoFaEnabled)} className={`px-4 py-2 text-xs font-mono font-medium rounded-lg transition-colors ${twoFaEnabled ? 'bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20' : 'bg-green-500/10 border border-green-500/20 text-green-400 hover:bg-green-500/20'}`}>
                ...
                <div className="space-y-3">
                  {connectedDevices.map((device, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 rounded-lg">
                      <div className="flex items-center gap-3">
                        {device.type === 'laptop' ? <Laptop className="w-5 h-5 text-white/40" /> : device.type === 'mobile' ? <Smartphone className="w-5 h-5 text-white/40" /> : <Monitor className="w-5 h-5 text-white/40" />}
                        <div>
                          <p className="text-sm text-white font-mono flex items-center gap-2">
                            {device.name}
                            {device.current && <span className="text-[10px] bg-green-500/10 text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded-full">Current</span>}
                          </p>
            ...
            <div className="space-y-6">
              <div className="bg-[#0a0a14] border border-[#1a1a2e] rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button className="flex items-center gap-2 px-4 py-2 bg-[#F7931A] text-black text-xs font-mono font-bold rounded-lg hover:bg-[#F7931A]/90 transition-colors">
                ...
                <div className="bg-[#F7931A]/5 border border-[#F7931A]/20 rounded-lg p-3 mb-4">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-[#F7931A]/80">API keys provide full access to your account. Never share your keys or commit them to version control.</p>
                  </div>
                </div>
                <div className="space-y-3">
                  {apiKeys.map((apiKey) => (
                    <div key={apiKey.id} className="p-4 bg-white/[0.02] border border-white/5 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] bg-green-500/10 text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded-full">{apiKey.status}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <button onClick={() => setShowKey(showKey === apiKey.id ? null : apiKey.id)} className="p-1.5 bg-white/5 hover:bg-white/10 rounded transition-colors">
                            {showKey === apiKey.id ? <EyeOff className="w-3.5 h-3.5 text-white/50" /> : <Eye className="w-3.5 h-3.5 text-white/50" />}
                          </button>
                          <button onClick={() => handleCopyKey(apiKey.id, apiKey.key)} className="p-1.5 bg-white/5 hover:bg-white/10 rounded transition-colors">
                            {copiedKey === apiKey.id ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5 text-white/50" />}
                          </button>
                          <button onClick={() => handleRevokeKey(apiKey.id)} className="p-1.5 bg-red-500/10 hover:bg-red-500/20 rounded transition-colors">
                      ...
                      <div className="flex items-center gap-4 text-[10px] text-white/30 font-mono mt-2">
            ...
            <div className="space-y-6">
              {checkoutSuccess && (
                <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg flex items-center gap-3">
                  ...
                  <button onClick={() => setCheckoutSuccess(false)} className="text-xs text-green-400/40 hover:text-green-400 font-mono">Dismiss</button>
                </div>
              )}
              ...
              <div className="bg-[#0a0a14] border border-[#1a1a2e] rounded-xl p-6">
                ...
                {tier === 'free' && !isActive ? (
                  <div className="text-center py-8">
                    ...
                    <button onClick={() => router.push('/pricing')} className="px-6 py-2.5 bg-[#F7931A] text-black text-sm font-mono font-bold rounded-lg hover:bg-[#F7931A]/90 transition-colors">
                      View Plans
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-lg">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] text-white/30 font-mono">Paid in BTC</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 rounded-lg">
                      ...
                      <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                        ...
                        status === 'expired' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' :
                    ...
                    {endDate && (
                      <div className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 rounded-lg">
                        <span className="text-xs text-white/50 font-mono">Expires</span>
              ...
              {tier !== 'free' && isActive && (
                <div className="bg-[#0a0a14] border border-[#1a1a2e] rounded-xl p-6">
                  ...
                  <p className="text-xs text-white/30 font-mono mt-3">
                    Subscriptions are non-recurring. To continue after expiration, make a new Bitcoin payment.
                  </p>
                </div>
              )}
            </div>
