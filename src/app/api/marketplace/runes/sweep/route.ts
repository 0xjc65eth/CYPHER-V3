  ...
  try {
    ...
    switch (action) {
      case 'psbt': {
        ...
        const data = await runesMarketService.getSweepingPsbt({
      ...
      case 'submit': {
        ...
        const data = await runesMarketService.submitSweeping({
    ...
    console.error('[API] POST /api/marketplace/runes/sweep error:', error);
    return NextResponse.json(
