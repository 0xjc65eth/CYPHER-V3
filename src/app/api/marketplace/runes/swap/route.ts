  ...
  try {
    ...
    switch (action) {
      case 'psbt': {
        ...
        const data = await runesMarketService.getSwapPsbt({
      ...
      case 'submit': {
        ...
        const data = await runesMarketService.submitSwap({
    ...
    console.error('[API] POST /api/marketplace/runes/swap error:', error);
    return NextResponse.json(
