...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'psbt') {
      ...
      const data = await ordinalsMarketService.getBatchListingPSBT({
        listings,
        makerFeeBp,
      });
      ...
      const data = await ordinalsMarketService.submitBatchListing({
        psbt,
        signatures,
      });
    ...
    console.error('[API] POST /api/marketplace/raresats/listing error:', error);
    return NextResponse.json(
