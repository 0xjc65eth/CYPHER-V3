...
import type { OrdinalsTokensParams } from '@/services/ordinalsMarketService';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const params: OrdinalsTokensParams = {};
    ...
    const data = await ordinalsMarketService.getTokens(params);
    return NextResponse.json({
      ...data,
      _source: 'gamma',
    });
  } catch (error) {
    console.error('[API] GET /api/marketplace/tokens error:', error);
    return NextResponse.json(
