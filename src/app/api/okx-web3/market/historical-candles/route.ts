...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await okxWeb3Service.getHistoricalCandles({
      chainIndex,
    ...
    console.error('[API] GET /api/okx-web3/market/historical-candles error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch historical candlestick data' },
      { status: 500 }
    );
  }
}
