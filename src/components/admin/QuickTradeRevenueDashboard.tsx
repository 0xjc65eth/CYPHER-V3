...
export function QuickTradeRevenueDashboard() {
  ...
  const fetchRevenueData = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/quick-trade/revenue/?wallet=0xAE3642A03a1e4bd7AB7D919d14C54ECf1BFdddd3');
      if (response.ok) {
  ...
  const exportData = async (format: 'csv' | 'json') => {
    setIsExporting(true);
    try {
      const response = await fetch('/api/quick-trade/revenue/', {
        method: 'POST',
